...
path_PC_025150=[direc name '/PC_bayes_025150.dat'];
path_PC_050200=[direc name '/PC_bayes_050200.dat'];
path_PC_100300=[direc name '/PC_bayes_100300.dat'];
path_PC_200400=[direc name '/PC_bayes_200400.dat'];
path_PC_300500=[direc name '/PC_bayes_300500.dat'];
lerr=dlmread(MFrates);
...
ll = size(lerr, 1);         % number of trials
llc  = ll - mod(ll, n);
...
%% compute TRACE estimates and bayesian theory
for j=1:ndelays
    idxt=find(timePC>Rini(j) & timePC<Rend(j));
    [te,tm,te_var]=BLS_timing(Iini(j),Iend(j),Rini(j),Rend(j),dt);
    TRACE(j).te=te;
    TRACE(j).tm=tm;
...
out.J=J';
out.Tsignaltype=rule;
out.learnparam=learnparam;
